...
function renderForecastExpenses(){const list=byId("forecastExpensesList");if(!list)return;const loanSpent=expensesData.filter(x=>x.estado!=="cancelado"&&x.pagado_por==="Préstamo").reduce((s,x)=>s+Number(x.importe||0),0);const total=forecastExpensesData.reduce((s,x)=>s+Number(x.importe_previsto||0),0);byId("forecastLoanTotal").textContent=euroV55.format(weddingLoan);byId("forecastLoanSpent").textContent=euroV55.format(loanSpent);byId("forecastTotal").textContent=euroV55.format(total);byId("forecastLoanRemaining").textContent=euroV55.format(weddingLoan-loanSpent-total);list.innerHTML=forecastExpensesData.length?forecastExpensesData.map(x=>`<article class="expense-card"><div><strong>${escapeHtml(x.concepto||"")}</strong><p>${escapeHtml(x.categoria||"Sin categoría")}${x.fecha_aproximada?` · ${x.fecha_aproximada}`:""}</p></div><strong>${euroV55.format(Number(x.importe_previsto||0))}</strong><div class="expense-actions"><button type="button" class="button button--secondary" data-forecast-edit="${x.id}">Editar</button><button type="button" class="button button--danger" data-forecast-delete="${x.id}">Eliminar</button></div></article>`).join(""):'<p class="empty-state">Todavía no hay gastos previstos.</p>';}
async function loadForecastExpenses(){try{forecastExpensesData=await api("/rest/v1/gastos_previstos_v55?select=*&order=created_at.desc");renderForecastExpenses();byId("forecastMessage").textContent="";}catch(e){byId("forecastMessage").textContent=e.message;}}
function resetForecastForm(){byId("forecastExpenseForm")?.reset();byId("forecastExpenseId").value="";byId("forecastCancelEdit").hidden=true;}
document.querySelectorAll("[data-economy-tab]").forEach(b=>b.addEventListener("click",()=>{const t=b.dataset.economyTab;document.querySelectorAll("[data-economy-tab]").forEach(x=>x.classList.toggle("is-active",x===b));byId("economyRealPanel").hidden=t!=="real";byId("economyForecastPanel").hidden=t!=="forecast";if(t==="forecast")loadForecastExpenses();}));
byId("forecastExpenseForm")?.addEventListener("submit",async e=>{e.preventDefault();const id=byId("forecastExpenseId").value,p={concepto:byId("forecastConcept").value.trim(),categoria:byId("forecastCategory").value.trim()||null,importe_previsto:Number(byId("forecastAmount").value||0),fecha_aproximada:byId("forecastDate").value||null,notas:byId("forecastNotes").value.trim()||null,updated_at:new Date().toISOString()};try{await api(id?`/rest/v1/gastos_previstos_v55?id=eq.${encodeURIComponent(id)}`:"/rest/v1/gastos_previstos_v55",{method:id?"PATCH":"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify(p)});resetForecastForm();await loadForecastExpenses();byId("forecastMessage").textContent=id?"Gasto previsto actualizado.":"Gasto previsto añadido.";}catch(err){byId("forecastMessage").textContent=err.message;}});
byId("forecastCancelEdit")?.addEventListener("click",resetForecastForm);byId("forecastExpensesList")?.addEventListener("click",async e=>{const ei=e.target.closest("[data-forecast-edit]")?.dataset.forecastEdit,di=e.target.closest("[data-forecast-delete]")?.dataset.forecastDelete;if(ei){const x=forecastExpensesData.find(y=>String(y.id)===String(ei));if(x){byId("forecastExpenseId").value=x.id;byId("forecastConcept").value=x.concepto||"";byId("forecastCategory").value=x.categoria||"";byId("forecastAmount").value=x.importe_previsto||"";byId("forecastDate").value=x.fecha_aproximada||"";byId("forecastNotes").value=x.notas||"";byId("forecastCancelEdit").hidden=false;}}if(di&&confirm("¿Eliminar este gasto previsto?")){try{await api(`/rest/v1/gastos_previstos_v55?id=eq.${encodeURIComponent(di)}`,{method:"DELETE",headers:{Prefer:"return=minimal"}});await loadForecastExpenses();}catch(err){byId("forecastMessage").textContent=err.message;}}});
