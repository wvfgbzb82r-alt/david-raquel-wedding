-- =============================================================
-- DAVID & RAQUEL · INSTALACIÓN ÚNICA DE SUPABASE
-- Copia TODO este archivo en Supabase > SQL Editor > New query
-- y pulsa RUN una sola vez.
-- =============================================================
...
-- Los invitados pueden subir, pero no listar ni descargar.
...
-- Solo el usuario administrador de los novios puede verlos.
...
-- Obliga a PostgREST a actualizar su esquema inmediatamente.
notify pgrst, 'reload schema';

-- =============================================================
-- 3. COMPROBACIÓN FINAL
-- Deben salir CINCO filas y todas deben poner OK.
-- =============================================================

select 'tabla_confirmaciones' as comprobacion,
       case when to_regclass('public.confirmaciones_v24') is not null
            then 'OK' else 'FALTA' end as estado
union all
select 'funcion_confirmacion',
       case when to_regprocedure('public.guardar_confirmacion_v24(jsonb)') is not null
            then 'OK' else 'FALTA' end
union all
select 'tabla_fotos',
       case when to_regclass('public.media_uploads_v24') is not null
            then 'OK' else 'FALTA' end
union all
select 'bucket_fotos',
       case when exists (
         select 1 from storage.buckets where id = 'wedding-media-v24'
       ) then 'OK' else 'FALTA' end
union all
select 'usuario_admin',
       case when exists (
         select 1 from auth.users
         where id = '3c36865b-a156-476f-82fd-7d95a3aea21b'::uuid
       ) then 'OK' else 'FALTA' end;
